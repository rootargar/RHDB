...
if (isset($_POST['exportar_excel'])) {
    ...
    if (!empty($empleado_seleccionado)) {
        ...
        $sql_empleado = "SELECT IdEmp, Empleado, Puesto, Departamento FROM capacitaciones WHERE IdEmp = ? GROUP BY IdEmp, Empleado, Puesto, Departamento";
    ...
    if (empty($empleado_seleccionado)) {
        ...
        $sql_empleados = "SELECT DISTINCT IdEmp, Empleado, Puesto, Departamento FROM capacitaciones ORDER BY Empleado";
...
function mostrarCursosFaltantes($conn, $idEmpleado, $empleado_info, $mostrarEmpleado = true, $esExcel = false) {
    ...
    $sql_cursos_requeridos = "SELECT cp.IdCurso, cp.NombreCurso, p.Area, cp.Departamento 
                              FROM cursoxpuesto cp
                              LEFT JOIN plancursos p ON cp.IdCurso = p.IdCurso
                              WHERE cp.puesto = ? 
                              ORDER BY cp.NombreCurso";
    $params_req = array($empleado_info['Puesto']);
    ...
    $sql_completados = "SELECT c.IdCurso
                        FROM capacitaciones c
                        INNER JOIN plancursos p ON c.IdCurso = p.IdCurso
                        WHERE c.IdEmp = ? 
    ...
    while ($req = sqlsrv_fetch_array($stmt_req, SQLSRV_FETCH_ASSOC)) {
        ...
        if (!in_array($req['IdCurso'], $cursos_completados)) {
            ...
            $sql_programado = "SELECT p.Estado 
                              FROM capacitaciones c
                              INNER JOIN plancursos p ON c.IdCurso = p.IdCurso
                              WHERE c.IdEmp = ? AND c.IdCurso = ?";
                    ...
                    <form method="post" action="" class="row g-3 align-items-center">
                        <div class="col-md-6">
                            ...
                            <select class="form-select select2" id="id_empleado" name="id_empleado">
                                ...
                                $sql_empleados = "SELECT IdEmp, Empleado FROM capacitaciones GROUP BY IdEmp, Empleado ORDER BY Empleado";
                                $stmt_empleados = sqlsrv_query($conn, $sql_empleados);
