...
if (isset($_POST['exportar_excel'])) {
    ...
    <body>
        <h2>Reporte de Próximas Capacitaciones (a partir del ' . date('d/m/Y', strtotime($fecha_seleccionada)) . ')</h2>
        <table>
            <thead>
                <tr>
                    <th>ID</th>
    ...
    // Obtener los datos de las próximas capacitaciones en SQL Server
    $sql = "SELECT Id, IdEmp, Empleado, Puesto, FechaIni, FechaFin, IdCurso, NomCurso, Area, Departamento,
            DATEDIFF(day, GETDATE(), FechaIni) as DiasRestantes
            FROM capacitaciones 
            WHERE FechaIni >= ? 
            ORDER BY FechaIni ASC";
            
    $params = array($fecha_seleccionada);
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#tabla-proximas').DataTable({
                ...
                order: [[10, 'asc']] // Ordenar por días restantes (ascendente)
            });
