...
if (isset($_POST['exportar_excel'])) {
    ...
    if ($stmt !== false) {
        while ($fila = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            ...
            $fechaCurso = !empty($fila['FechaCurso']) ? $fila['FechaCurso'] : 'N/A'; // FechaCurso ahora es NVARCHAR
            ...
            echo '<td>' . (!empty($fila['Costo']) ? $fila['Costo'] : '0.00') . '</td>';
                        ...
                        <tbody>
                            ...
                            if ($stmt !== false) {
                                while ($fila = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                    ...
                                    echo '<td>' . $fila['Instructor'] . '</td>';
                                    echo '<td>' . $fila['Proveedor'] . '</td>';
                                    echo '<td class="currency">' . (!empty($fila['Costo']) ? $fila['Costo'] : '0.00') . '</td>';
                                    echo '<td>' . $fila['NumParticipantes'] . '</td>';
                                    echo '<td>' . $fila['Factura'] . '</td>';
                                    echo '<td>' . $fila['Estado'] . '</td>';
                                    echo '</tr>';
                                ...
                                // Opcional: Mostrar error si hay uno
                                if (($errors = sqlsrv_errors()) != null) {
                                    echo '<tr><td colspan="15" class="text-center text-danger">';
                                    foreach ($errors as $error) {
                                        echo "Error SQLSTATE: " . $error['SQLSTATE'] . ", Código: " . $error['code'] . ", Mensaje: " . $error['message'] . "<br />";
                                    }
                                    echo '</td></tr>';
                                }
                            }
